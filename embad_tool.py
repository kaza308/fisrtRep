...
def return_corner(location):
    # org=np.array([])
    org=location[3]
    org=np.vstack((org, location[2]))
    org=np.vstack((org, location[0]))
    return org
def crop(image,loc):
...
#                   [113,491]])
    ...
    print "hhhhhh", location,type(location)
    srcpt=np.float32(return_corner(location))
# M=cv2.getAffineTransform(srcpt,wikipt)
    M=cv2.getAffineTransform(wikipt,srcpt)

    dst=cv2.warpAffine(wiki,M,(w,h))
#cv2.imshow("ddd",dst)
